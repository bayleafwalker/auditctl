"""Validate the exact auditctl wheel selected for a GitHub release."""
...
AUDITCTL_WHEEL_RE = re.compile(r"^auditctl-(?P<version>[^-]+)-.+\.whl$")
...
def validate_wheel(wheel_path: Path, tag: str | None = None) -> None:
    ...
    wheel_match = AUDITCTL_WHEEL_RE.fullmatch(wheel_path.name)
    if wheel_match is None:
        raise AssertionError(f"wheel is not an auditctl wheel: {wheel_path.name}")
    ...
    if metadata["Name"] != "auditctl":
        raise AssertionError(f"wheel metadata name is not auditctl: {metadata['Name']!r}")
    version = metadata["Version"]
    if version != wheel_match.group("version"):
        raise AssertionError("wheel filename and metadata versions differ")
    expected_tag = f"auditctl-v{version}"
